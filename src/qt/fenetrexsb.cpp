...
FenetreXSB::FenetreXSB()
{

}

// Constructeur
FenetreXSB::FichierXSB(std::string nom) {
...
void fenetrexsb::charger(std::string const& fichier) {
...
int fenetrexsb::nb_niv() const {
    return m_niveaux.size();
}

Menu fenetrexsb::as_menu() const {
    Menu menu;
    int i = 0;
...
std::vector<Niveau> const& fenetrexsb::niveaux() const {
    return m_niveaux;
}
